...
namespace KY
{
	namespace DX
	{
		bool Dx11Texture2D::Create(const Texture2DParam &param, const ResourceData &resData)
		{
			...
			return (SUCCEEDED(device->CreateTexture2D(&desc, data.pSysMem ? &data : nullptr, &mInternal)));
		}

		bool Dx11Texture2D::Create(const TextureParam &param, TextureAssertLoader *loader)
		{
			BOOST_ASSERT(loader);
